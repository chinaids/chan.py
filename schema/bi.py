...
class Bi(BaseModel):
    idx: int = -1
    beg_unit: MergedKLineUnit
    end_unit: MergedKLineUnit
    type: BiType = BiType.STRICT
    finished: bool = False

    @property
    def direction(self):
        if self.beg_unit.fenxing == FenxingType.BOTTOM:
            return BiDirection.UP
        elif self.beg_unit.fenxing == FenxingType.TOP:
            return BiDirection.DOWN
        raise ValueError(f'bi starts with a wrong fenxing: {self.beg_unit.fenxing}')

    @property
    def begin_value(self):
        return self.beg_unit.low if self.direction == BiDirection.UP else self.beg_unit.high

    @property
    def end_value(self):
        return self.end_unit.high if self.direction == BiDirection.UP else self.end_unit.low

    @property
    def high_value(self):
        return self.end_unit.high if self.direction == BiDirection.UP else self.beg_unit.high

    @property
    def low_value(self):
        return self.beg_unit.low if self.direction == BiDirection.UP else self.end_unit.low

    @property
    def beg_single_unit(self) -> KLineUnit:
        if self.direction == BiDirection.UP:
            return self.beg_unit.get_peak_klu(is_high=False)
        else:
            return self.beg_unit.get_peak_klu(is_high=True)

    @property
    def end_single_unit(self) -> KLineUnit:
        if self.direction == BiDirection.UP:
            return self.end_unit.get_peak_klu(is_high=True)
        else:
            return self.end_unit.get_peak_klu(is_high=False)

    @property
    def kline_cnt(self):
        return self.end_unit.idx - self.beg_unit.idx + 1

    @model_validator(mode='after')
    def validate_price(self):
        if self.direction == BiDirection.DOWN:
            assert self.beg_unit.high > self.end_unit.low, f'price err when going down, {self.beg_unit.high}, {self.end_unit.low}'
        else:
            assert self.beg_unit.low < self.end_unit.high, f'price err when going up, {self.beg_unit.high}, {self.end_unit.low}'
